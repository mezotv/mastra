...
function requestContext(overrides: Partial<{ threadId: string; scope: string; authenticated: boolean }> = {}) {
  ...
  context.set('controller', {
    ...
    getState: () => ({ githubProjectId: PROJECT_ID }),
  });
  return context;
}

async function prepare(storage: WorkItemsStorageInMemory, role = 'work') {
...
describe('FactoryPhaseStateProcessor', () => {
  ...
  it('moves an approved plan to Building before emitting the next phase signal', async () => {
    ...
    await expect(storage.get('org-1', PROJECT_ID, prepared.item.id)).resolves.toMatchObject({
      revision: 2,
      stages: ['execute'],
    });
    ...
    await storage.prepareRunStart({
      ...
      workItem: {
        ...
        input: {
          ...
          sourceKey: prepared.item.sourceKey,
