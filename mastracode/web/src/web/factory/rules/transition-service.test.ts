...
async function createItem(
  storage: WorkItemsStorageInMemory,
  ...
  return (
    await storage.upsert({
      ...
      input: {
        ...
        sourceKey: overrides.sourceKey ?? 'github-issue:1',
        title: 'Fix the bug',
        url: null,
...
describe('FactoryTransitionService', () => {
  it('runs onExit before onEnter and atomically persists accepted decisions', async () => {
    ...
    expect((await storage.get('org-1', PROJECT_ID, item.id))?.stageHistory.map(entry => entry.stage)).toEqual([
      'intake',
      'execute',
    ]);
  ...
  it('always returns typed stale on CAS loss and never overwrites canonical state', async () => {
    ...
    await storage.update('org-1', item.id, 'user-2', { title: 'Changed concurrently' });
  ...
  it('replays immutable ingress across rule version changes without re-evaluation', async () => {
    ...
    expect((await storage.get('org-1', PROJECT_ID, item.id))?.stages).toEqual(['execute']);
  });
  ...
  it('accepts unchanged-stage no-op without revising history', async () => {
    ...
    expect((await storage.get('org-1', PROJECT_ID, item.id))?.stageHistory).toHaveLength(1);
  });
