...
Objective records and the `setObjective` / `updateObjectiveOptions` options include an optional `activeDurationMs` value for user interfaces that track active pursuit time. Mastra stores the supplied value but doesn't advance it automatically. Missing values represent zero, and the duration measures active work rather than the goal's wall-clock age.
